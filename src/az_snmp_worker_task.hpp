...
inline void WorkerTask(
    ...
    try {
        ...
        auto [oid, req_type] = handler.process_request(context->raw_data);

        // Read value from the MIB (via injected interface)
        std::string mib_value = mib_service->read(oid);

        // Serialize the Response PDU
        std::vector<uint8_t> response_data = handler.resp_get(context->raw_data, oid, mib_value);
